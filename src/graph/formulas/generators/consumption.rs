...
use super::super::{expr::Expr, fallback::FallbackExpr};
    ...
    E: Edge,
{
    ...
    fn component_consumption(&mut self, component_id: u64) -> Result<Expr, Error> {
        ...
        if component.is_meter() {
            ...
            // the expression.
            let mut successors = BTreeSet::from_iter(
                self.graph
            ...
            for sibling in self.graph.siblings_from_successors(component_id)? {
                ...
                for successor in self.graph.successors(sibling.component_id())? {
                    successors.insert(successor.component_id());
                }
            }

            // Subtract each successor from the expression.
            for successor in successors {
                expr = expr - self.meters_with_fallback(successor)?;
            }
    ...
    fn meters_with_fallback(&self, successor_id: u64) -> Result<Expr, Error> {
        let successor = self.graph.component(successor_id)?;
        if successor.is_meter() {
            Ok(FallbackExpr {
                prefer_meters: true,
                graph: self.graph,
            }
            .generate(vec![successor.component_id()])?)
        } else {
            Ok(Expr::from(successor))
        }
    }

    /// Generates the consumption formula for the given node.
    pub fn consumption_formula(&mut self) -> Result<String, Error> {
        let mut all_meters = None;
        while let Some(meter_id) = self.unvisited_meters.iter().next().copied() {
...
mod tests {
    ...
    #[test]
    fn test_consumption_diamond_meters() -> Result<(), Error> {
        let mut builder = ComponentGraphBuilder::new();
        let grid = builder.grid();
